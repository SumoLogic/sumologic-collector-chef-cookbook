...
action :install do
  if @current_resource.installed
    ...
    installer_filename = node['platform_family'] == 'windows' ? 'sumocollector.exe' : 'sumocollector'
    installer_args = build_args
    ...
    execute 'Install Sumo Collector' do
      command "#{Chef::Config[:file_cache_path]}/#{installer_filename} #{installer_args}"
      timeout 3600
    end
